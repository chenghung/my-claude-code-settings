本 rule 的目的是統一 `agents/` 目錄下所有 subagent 定義檔的結構，使 main agent 能以一致的方式解析各 subagent 的職責與行為。本 rule 與 `prompt-quality-checks.md` 互補：後者規定設計原則與品質檢查，本 rule 規定具體章節格式。

## Applicability

- 適用對象：`agents/` 目錄下的所有 subagent 定義檔
- 不適用：`skills/` 與 `rules/` 目錄（可參考本 rule 的結構，但不強制相同）
- 觸發時機：新增 subagent 或修改既有 subagent 的章節結構時，必須遵循本 rule
...
- 章節標題一律使用**英文**，即使內文是繁體中文（例如：標題寫 `## In Scope`，條列內容可以是中文）
...
- `prompt-quality-checks.md`：規定 prompt 品質的八條設計原則與五項品質檢查；本 rule 是其形式層面的具體落實，兩者應同時滿足
- `git-commit-style.md`：修改 `agents/` 目錄下的 subagent 定義檔時，commit message 的 scope 使用 `agent`

## Trigger for Compliance Check

以下情況發生時，必須對該 subagent 檔案執行本 rule 的合規檢查：

- 新建 subagent 定義檔
- 修改既有 subagent 的章節結構（新增、刪除、重命名章節）
- 在 `prompt-quality-checks.md` 的觸發條件下進行品質檢查時，本 rule 同步執行
