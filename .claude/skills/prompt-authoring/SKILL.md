...
- **平行委派三個審查端 subagent**：撰寫完成後，同時委派下列三者，各自只取得目標檔案與自身視角的判準，不得取得其他審查者的 findings，以維持視角互相盲目：
  - `prompt-compliance-reviewer`：內容軸合規、重複、幻覺引用、措辭軸、skill 漸進載入、單一來源。
...
- 安全邊界缺失
- 假的保證：宣稱強制執行，但沒有 hook 或 permission 設定支撐
- 規則之間直接衝突
- 幻覺引用：引用不存在的檔案、工具或功能
...
- 迭代上限 5 輪。
- 每輪之後，must-fix 數量必須較前一輪下降。
- 一旦 must-fix 數量停滯不再下降，或跑滿 5 輪仍有剩餘 must-fix，停止迭代，回報剩餘 findings 交人類判斷，不再自行嘗試。
