...
- **委派撰寫**：實際的撰寫工作委派給 `prompt-author`，呼叫方不自行編輯 prompt 定義檔。
...
- **合併去重後交回撰寫端**：三者回傳的 findings 由呼叫方合併，指向同一位置且同一問題的重複 findings 併為一則，再交回 `prompt-author` 處理。
...
- 迭代上限 5 輪，每跑一次審查算一輪。
- **收斂出口**：某一輪審查回傳的 must-fix 為 0，且該輪沒有任何 finding 被採納並實際改檔，即判定收斂，回報結果並結束迭代。第 1 輪就達成此條件同樣算收斂，不視為失敗。
- **採納後的驗證輪**：must-fix 為 0 但該輪有任何 finding 被採納並實際改檔時（不論其分數是否達預設門檻），改完後再跑一輪審查驗證改動。該輪 must-fix 仍為 0 即結束迭代，不論是否又出現新的 nice-to-have——nice-to-have 不構成繼續迭代的理由。該輪若出現新的 must-fix，修正後再驗證一次，仍未歸零則走失敗出口。
- **改檔判定依據**：上述兩條所稱「有 finding 被採納並實際改檔」，一律以撰寫端該輪回傳的異動檔案清單認定——清單為空即為否，清單含任一檔案即為是；不以採納宣告或 findings 數量推測。
