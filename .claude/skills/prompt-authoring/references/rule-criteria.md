...
自我檢測：這條規則的關鍵詞，在同層級的其他檔案裡搜得到第二次命中嗎？搜到即為 finding；若命中落在上述三個章節內，進一步問——這段重複的內容，在判準檔中找得到一個它可以改為指向的對應章節嗎？找得到即為 finding，應改為指向判準檔；找不到（該內容純屬定義檔結構本身要求填寫的欄位描述，判準檔沒有規定其具體文字），才屬豁免範圍，不算 finding。
