...
| `github-manager` | 透過 gh 對 GitHub issue、PR、comment 的操作，除 issue 或 PR 標題與內文的建立或修改之外，其餘一律涵蓋（含唯讀查詢、狀態轉換如關閉或重開、留言張貼、中繼資料操作、審查決議與 merge）；該例外已改由 `issue-pr-publisher` 統籌，main agent 不再為此直接委派本列，但該 orchestrator 內部仍會派工本列執行實際的 gh 指令，本列的執行能力並未減少；只負責執行，不負責審查回覆的結構化組裝 |
