...
- **內容檔路徑**：建立與更新操作若帶有內容，main agent 先把完整內容寫進符合 tmp-file-usage 規範的暫存檔，prompt 只放該檔案路徑，不放內文本身。純 metadata 操作（改標題、權限、刪除、搜尋、匯出）不需要內容檔
- **其他條件**：搜尋關鍵字或其他使用者指定的篩選條件

上傳用的暫存檔屬於內容傳輸緩衝，沿用匯出快取檔由流程直接寫的既有慣例，不經一般 markdown 編輯路由。
