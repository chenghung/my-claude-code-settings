...
| 事件產生器掛不起來（掛上就立刻串流結束，重掛也立刻結束），或某個 phase 完全收不到任何事件——既沒有事件行，也不再被自動推進 | 第一個該查的地方是環境裡有沒有殘留的匯出值，三個都是「安靜地把通道關掉」而不是報錯：`EO_GENERATOR_NO_MAIN` 讓產生器只定義完函式就以 0 結束、什麼都不印，連環境前提那個結束碼 3 都拿不到；`EO_GENERATOR_DRY_RUN` 讓自動推進完全不呼叫下行、改在 stderr 印觀察行，而產生器仍把它記成成功，於是那個 phase 無聲停擺；`EO_PHASE_POLL_SECONDS` 被設成一個大值時，新派工的 phase 在那段時間裡不在監看清單上。先分辨一件事再往這裡查：串流結束之前若收到過一行 `STATE-FILE-GONE`，成因是狀態檔在監看途中消失、不在這三個旗標上，處置見 `SKILL.md`「事件處置」那一列（那一種重掛之後不會立刻再結束，而是安靜下來，與這三個的症狀分得開） | 為什麼機制層收進腳本 |
...
三個值定案如下，逐字照抄自規格附錄 B，都是未查證推估：自動推進上限 **40 次**；`state_change_seq` 連續 **25 分鐘**沒有變化才印 SPINNING；狀態連續 **5 輪** 60 秒掃描都是 `unknown` 才印 UNCLASSIFIED。三者都以具名常數定義在 `scripts/lib/common.sh`（`EO_AUTO_PUSH_LIMIT`、`EO_SPINNING_SECONDS`、`EO_UNCLASSIFIED_ROUNDS`，掃描間隔另有 `EO_UNCLASSIFIED_SCAN_INTERVAL_SECONDS`），改值只改那一處。第一次真實跑 epic 就是它們的校準回合：跑完要回頭處理這三個值，而「處理」的意思是各自依本次拿得到的觀測材料重新定一次，包含判定原值可用而維持原值——維持原值也是一次校準結果，不是跳過。三個值拿得到的材料不一樣，逐項的完成判準見本節末段。
...
另兩個門檻各自也要記下一個觀測量，否則校準回合跑完只有一個門檻真的被校準過；而這兩個量拿不拿得到，答案不一樣，下面那段完成判準因此分兩半寫。SPINNING 那一個要記的是「真實 phase 在做事時，`state_change_seq` 最長多久沒有變化」——那是 25 分鐘的下界，門檻必須大於它——但這個量沒有任何落地位置：它的來源是低頻掃描子行程裡那兩個行程內關聯陣列（`event-generator.sh` 裡「常駐迴圈用的行程內狀態」那一段註解就寫著它們不落地狀態檔、隨行程結束而消失），既不寫狀態檔也不印出來，產生器一結束就沒了。所以它只在門檻真的被踩到、印出 SPINNING 的那一次留得下痕跡；門檻沒被踩到——也就是原值可用的那種正常情況——反而什麼痕跡都不剩。UNCLASSIFIED 那一個記「實際觀察到的連續 `unknown` 輪數上限」，一輪 60 秒（見 `EO_UNCLASSIFIED_SCAN_INTERVAL_SECONDS`），5 輪必須大於那些會自行離開 `unknown` 的短暫情形；這一個落在狀態檔的 `unknown_rounds` 欄位，拿得到，但要有人去讀，收集義務寫在 `SKILL.md`「事件處置」談持有旗標那幾段與「生命週期」第 6 步。兩者的失敗形態同形，都是誤報或漏報而不是流程中斷：太緊會對正在做事的 phase 一再派調查者，太鬆則讓真的卡住的 phase 拖很久才被發現。

校準回合的完成判準因此不是「跑過就算」，但也不是一句「三個值各指得出一個觀測值」——三個值拿得到的東西不一樣，判準分兩半：

- **自動推進上限與 UNCLASSIFIED 輪數**：兩個量都落在狀態檔（`auto_push_count`、`unknown_rounds`），收集義務已經寫進 `SKILL.md`——編排端每次處理某個 phase 的事件時順手讀一次、收尾關 tab 之前再讀最後一次，留整個 epic 看過的最大值（見「事件處置」談持有旗標那幾段與「生命週期」第 6 步；記錄一旦被關 tab 移出狀態檔，那兩個值就永久消失）。這兩個各要指得出讀到的最大值，並記下該門檻對應的事件（`AUTO-PUSH-LIMIT`、`UNCLASSIFIED`）本次有沒有印過，據此各自記下採用新值或維持原值。印過，那個值就是門檻真的被踩到的證據；沒印過，讀到的最大值是下界而不是峰值——編排端只在被事件喚醒時讀得到，而兩個欄位又各自在自己的歸零點被清掉，中間的峰值沒有痕跡。所以「沒印過、而最大值遠低於門檻」要記成「本回合只取到下界、該值維持原值」，不得記成「已證實原值夠用」。
- **SPINNING 秒數**：這一個的觀測值只在本次跑出過 SPINNING 事件時才拿得到，理由見上一段——那個量只存在行程記憶體。跑完沒有出現過 SPINNING，就記下「本回合未取得觀測值、該值維持推估」，而不是把它算成已校準。不為它新增落地機制：那要動產生器，而收益是一個可能永遠不出現的觀測值。

任一項指不出上面要求它指出的東西，這個回合就還沒跑完，那個門檻也不得從未查證推估的名單上劃掉——三個值裡有兩個沒被校準過卻被當成已校準，是這一節最可能出現的失敗。
