...
#
# ---- 誰能用哪一種模式 ----
# investigator 兩種模式都用：預設模式讀畫面全文，用來理解對方卡在哪
# 裡；--marker-only 讀回完整標記行，從中取 seq——它需要 seq 才算得出
# 決策請求檔的路徑，而派工不帶 seq（事件行只有 phase 編號、停下狀態
# 與狀態關鍵字三項）。event-generator.sh 的邊緣觸發只抓標記行
# （--marker-only），拿新的 seq 判斷有沒有進展；orchestrator 兩種模
# 式都不呼叫本腳本——它只透過 phase-status.sh 看 herdr 自己的狀態分
# 類，不需要、也不應該自己解讀畫面文字。
#
