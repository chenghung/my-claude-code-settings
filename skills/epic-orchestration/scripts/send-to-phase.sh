...
#     結束，不印 handshake= 行；訊息只含 herdr 回應裡 error.code 與
#     error.message 兩個純字串欄位，不轉發回應原文（理由見下方對應程
#     式碼旁的註解）。
...
# workspace 守衛：本腳本與 press-approval.sh 是唯二會對一個活著的
...
# 時，兩邊的狀態檔與 agent 名稱會重合，跨 workspace 誤送在這裡不是理
...
case "$error_code" in
  ...
  *)
    ...
    # terminal_title／terminal_title_stripped，見 press-approval.sh 檔
    # 頭）。但 `agent_blocked` 這一類錯誤酬載會不會也帶 agent 物件未查
    # 證——要製造一個 blocked 的 agent 本身就是副作用，沒有無副作用的
    # 探測手段可用。因此不逐一查證每種錯誤碼的酬載形狀，改採結構性做
    # 法：只白名單擷取 code 與 message，其餘欄位一律不觸碰。兩個欄位
    # 任一取不到都給明確的替代字串，不靜默留空、也不因此退回轉發原文
    # ——退回原文等於這裡的收斂沒有意義。
    ...
    eo_die 6 "send-to-phase.sh: herdr 以結束碼 1 拒絕 agent prompt（target=$target），錯誤碼非逾時類：code=$error_code message=$error_message"
    ;;
esac
