...
# `done` 兩個值、兩個都要接受。理由只取實測到的那一層：實測到的落點是
# `done`（只等 `idle` 的那次等待在 25042 毫秒逾時，而狀態早已是
# `done`），所以只等 `idle` 會等到逾時、拿到 7。至於 `idle` 與 `done`
# 為什麼這樣分佈（例如「差別在那個 tab 有沒有被使用者點進去看過」），
# 那是幾次樣本支持的最合理解釋、不是查證過的機制，這裡不據它推論。
#
# 兩種都收在那個解釋對或錯的兩種情況下都安全，這才是這個判斷真正的依
# 據：解釋若對，`idle` 在無人值守下不會出現，多收一種不花成本；解釋若
# 錯，`idle` 隨時可能出現，而它已經被收在裡面了。危險的是反向——把
# `idle` 從這個清單裡拿掉（見 SKILL.md 與 references/rationale.md）。
#
