...
generate_command_skills() {
  ...
  while IFS= read -r -d '' md_path; do
    ...
    if [ -e "$out_path" ] && ! head -3 "$out_path" | grep -q 'GENERATED by install.sh'; then
      # Two different reasons an entry can occupy this path with no banner:
      # (1) it is the symlink link_items_into just made to a repo skill of
      #     the same flattened name — benign, since that skill already ships
      #     its own Antigravity slash command, so the generated duplicate is
      #     correctly skipped; (2) it is genuinely hand-authored by the user,
      #     which is the case the original WARNING protects. Tell them apart
      #     so the log doesn't cry wolf on (1).
      if [ -L "$out_dir" ] || [ -d "${REPO_DIR}/skills/${flat}" ]; then
...
deploy_antigravity() {
  ...
  # bridge the result into the dir agy actually reads.
  install_external_skills "antigravity-cli"
  link_items_into "${GEMINI_HOME}/antigravity-cli/skills" "${AG_CONFIG_DIR}/skills"
