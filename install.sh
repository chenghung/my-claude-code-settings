...
# For opencode, configured declaratively via opencode.json (not runtime).
...
install_superpowers() {
  ...
  case "$platform" in
    ...
    codex)
      ...
      ;;
    opencode)
      printf '  OK       superpowers configured via opencode.json plugin array\n'
      count_ok=$(( count_ok + 1 ))
      ;;
    *)
