...
#   - generate_command_skills' prune loop: `rm -rf` on an orphaned generated
#     command-skill directory. Double-fenced: the candidates come from
#     `find -type d` (which never matches a symlink, so a symlinked skill dir
#     is never a candidate) and are removed only when their own SKILL.md
#     still carries this converter's GENERATED banner (so a hand-authored or
#     externally-owned directory can never be a candidate either).
...
AG_CONFIG_DIR="${GEMINI_HOME}/config"
...
# ---------------------------------------------------------------------------
# codegraph MCP server (Antigravity CLI). Same shape as register_codegraph_mcp
# for Claude Code: ask the platform's own CLI rather than editing its config
# file, so the script and agy never keep two copies of the same JSON.
# `agy mcp list` prints a table (or "No MCP servers configured."), so the
# idempotency check greps for a row starting with the server name.
# The bare `--` before the command is required: without it agy parses `--mcp`
# as one of its own flags.
# ---------------------------------------------------------------------------
register_codegraph_mcp_antigravity() {
  if ! command -v agy > /dev/null 2>&1; then
    printf '  WARNING  agy CLI not found - skipping codegraph MCP server registration.\n'
    return
  fi

  # Captured via command substitution (not `agy mcp list | grep -q`) so a
  # match found early in the output can never make grep -q close its stdin
  # while agy is still writing: under this script's `set -o pipefail`, that
  # SIGPIPEs agy and its non-zero (signal) exit status becomes the pipeline's
  # overall status even though the match WAS found — sending this function
  # down the "not registered" branch and running `agy mcp add` on an
  # already-registered server. Same trap as tests/test-cli-aliases.sh's
  # assert_no_codegraph_install; current output is small enough not to
  # trigger it, but that is incidental, not structural. `|| true` guards the
  # assignment itself: agy exiting non-zero here must not abort this script
  # under `set -e` — an empty capture already reads as "not registered" below,
  # same as before.
  local mcp_list_output
  mcp_list_output="$(agy mcp list 2>/dev/null)" || true

  if grep -q '^codegraph[[:space:]]' <<< "$mcp_list_output"; then
    printf '  OK       codegraph MCP server already registered (agy)\n'
    count_ok=$(( count_ok + 1 ))
  elif agy mcp add codegraph -- codegraph serve --mcp; then
    printf '  CREATED  codegraph MCP server registered (agy)\n'
    count_created=$(( count_created + 1 ))
  else
    printf '  WARNING  failed to register codegraph MCP server with agy - skipping.\n'
    count_skipped=$(( count_skipped + 1 ))
  fi
}
...
#
# Non-destructive, like link_one: if <out> already exists as a real (non-
# symlink) file with no GENERATED banner, it is reported as a CONFLICT and
# left untouched rather than overwritten. This matters beyond Codex/opencode's
# AGENTS.md (repo-only paths): Antigravity's own Customizations panel writes
# to this same GEMINI.md path when a user clicks "+ Global", so real
# hand-authored content can already be sitting there.
generate_agents_md() {
  local out="$1"
  local platform="${2:-}"

  if [ -e "$out" ] && [ ! -L "$out" ] && ! head -1 "$out" | grep -q 'GENERATED by install.sh'; then
    printf '  CONFLICT %s\n           exists as a real file with no GENERATED banner — skipping to avoid destroying hand-authored content\n' "$out"
    count_skipped=$(( count_skipped + 1 ))
    return
  fi
...
# Generate one Antigravity skill per commands/**/*.md into
# ${AG_CONFIG_DIR}/skills, flattening nested paths with a hyphen
# (commands/vf/x.md -> vf-x), then prune orphaned generated skills.
#
# This directory holds three kinds of entry: symlinks to the repo's own
# skills/, the generated command-skills written here, and symlinks bridged in
# from the external-skills install. Pruning is therefore doubly fenced: it only
# considers real directories (find -type d never matches a symlink), and only
# removes one whose SKILL.md carries this converter's own banner text.
generate_command_skills() {
  if ! command -v python3 > /dev/null 2>&1; then
    printf '  WARNING  python3 not found - skipping Antigravity command-skill generation.\n'
    return
  fi

  local src_dir="${REPO_DIR}/commands"
  local dst_dir="${AG_CONFIG_DIR}/skills"
  local converter="${REPO_DIR}/scripts/md-command-to-antigravity-skill.py"
  [ -d "$src_dir" ] || return 0
  mkdir -p "$dst_dir"

  local -A wanted=()
  while IFS= read -r -d '' md_path; do
    local rel flat out_dir out_path
    rel="${md_path#"$src_dir"/}"
    flat="${rel//\//-}"
    flat="${flat%.md}"
    wanted["$flat"]=1
    out_dir="${dst_dir}/${flat}"
    out_path="${out_dir}/SKILL.md"

    # Guard 1: out_dir is itself a symlink — almost always the one
    # link_items_into just made to a repo skill of the same flattened name.
    # This must run before anything looks at out_path: if the symlinked
    # target happens to carry no SKILL.md yet, `[ -e "$out_path" ]` below is
    # false, so the check used to fall through to `mkdir -p "$out_dir"` (a
    # no-op on an already-existing dir) and then write SKILL.md straight
    # through the symlink into whatever external directory it points at —
    # and prune can never clean that up (find -type d skips symlinks by
    # design). So a symlinked out_dir is always skipped here, independent of
    # what (if anything) lives at the far end.
    if [ -L "$out_dir" ]; then
      printf '  SKIP     %s duplicates the repo skill "%s" - that skill already provides its own slash command, so no separate command-skill is generated.\n' "$out_path" "$flat"
      continue
    fi

    if [ -e "$out_path" ] && ! head -3 "$out_path" | grep -q 'GENERATED by install.sh'; then
      # out_dir is a real (non-symlink) directory whose SKILL.md carries no
      # banner. Two different reasons: (1) link_one hit a CONFLICT earlier
      # and left a pre-existing real directory of the same flattened name as
      # a repo skill in place (so no symlink was ever created there, and the
      # guard above never triggered) — still benign, since that skill ships
      # its own Antigravity slash command; (2) it is genuinely hand-authored
      # by the user, which is the case the WARNING protects. Tell them apart
      # so the log doesn't cry wolf on (1).
      if [ -d "${REPO_DIR}/skills/${flat}" ]; then
        printf '  SKIP     %s duplicates the repo skill "%s" - that skill already provides its own slash command, so no separate command-skill is generated.\n' "$out_path" "$flat"
      else
        printf '  WARNING  %s already exists and is hand-authored (no GENERATED banner) - skipping to avoid destroying it.\n' "$out_path"
      fi
    ...
    mkdir -p "$out_dir"
    if python3 "$converter" "$md_path" "$flat" > "${out_path}.tmp" 2>/dev/null; then
      mv "${out_path}.tmp" "$out_path"
      printf '  GENERATED %s\n' "$out_path"
    else
      rm -f "${out_path}.tmp"
  ...
  while IFS= read -r -d '' skill_dir; do
    ...
    if [ -z "${wanted[$base]:-}" ] \
      ...
      && head -3 "${skill_dir}/SKILL.md" | grep -q 'GENERATED by install.sh from commands'; then
      rm -rf "$skill_dir"
      printf '  PRUNED   %s (no repo source)\n' "$skill_dir"
    fi
  done < <(find "$dst_dir" -maxdepth 1 -mindepth 1 -type d -print0 | sort -z)
}

# Generate one Antigravity agent .md per agents/*.md into
# ${AG_CONFIG_DIR}/agents, then prune orphaned generated files. Banner
# detection scans the first few lines because the banner sits on line 2,
# inside the YAML frontmatter whose '---' fence must be line 1.
generate_antigravity_subagents() {
  if ! command -v python3 > /dev/null 2>&1; then
    printf '  WARNING  python3 not found - skipping Antigravity subagent generation.\n'
    return
  fi

  local agents_src="${REPO_DIR}/agents"
  local agents_dst="${AG_CONFIG_DIR}/agents"
  local converter="${REPO_DIR}/scripts/md-agent-to-antigravity.py"
  mkdir -p "$agents_dst"
...
deploy_antigravity() {
  printf 'Deploying to Antigravity CLI\n  target: %s\n\n' "$GEMINI_HOME"
  mkdir -p "$GEMINI_HOME" "${AG_CONFIG_DIR}/skills" "${AG_CONFIG_DIR}/agents"

  link_items_into "${REPO_DIR}/skills" "${AG_CONFIG_DIR}/skills"
  generate_command_skills

  # No size warning here, unlike the Codex branch. Antigravity's documented
  # 12000-character limit for a rules file is an editor-side constraint: agy
  # 1.1.22 loads ~/.gemini/GEMINI.md in full, verified with a 15132-character
  # file whose trailing content was still reachable in the model's context.
  generate_agents_md "${GEMINI_HOME}/GEMINI.md"

  generate_antigravity_subagents

  # The skills CLI has no agent id that writes into ${AG_CONFIG_DIR}/skills:
  # antigravity-cli lands in ~/.gemini/antigravity-cli/skills instead. Install
  # under that id (correct scope, and the CLI keeps managing versions), then
  # bridge the result into the dir agy actually reads. The bridge itself must
  # also obey --no-external: without this guard, a user who explicitly opted
  # out of external skills would still get any pre-existing ones under
  # antigravity-cli/skills wired into the dir agy reads, while the other
  # three platforms stay fully inert under --no-external — this keeps the
  # flag's meaning consistent across all four.
  ...
  if [ -L "${GEMINI_HOME}/AGENTS.md" ]; then
    printf '  NOTE     %s is a hand-made symlink that predates this deploy.\n' "${GEMINI_HOME}/AGENTS.md"
    printf '           Rules now ship via %s; remove it whenever convenient.\n' "${GEMINI_HOME}/GEMINI.md"
  fi
}
...
for arg in "$@"; do
  case "$arg" in
    ...
    --all)    want_claude=1; want_codex=1; want_opencode=1; want_antigravity=1 ;;
