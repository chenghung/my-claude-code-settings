...
本回合的請求檔讀得到（哪一份算本回合的，閘門見 `In Scope`：認的是標記行的 `state` 值，不是這次的派工事由）、而它的「問題」欄位問的是一個已經執行完的動作要怎麼收時，推薦中另須以自己的話交代下面兩件事。不論派工事由是 `need-decision` 還是標記缺席，這一項一律適用——兩者會分岔，而分岔的那一種正是最需要它的：phase agent 在 `state=need-decision` 後面多接了說明文字（契約明文禁止），事件產生器的狀態白名單就把那一輪判成標記缺席，編排端因此以標記缺席派工，而標記行讀回來的 `state` 仍是 `need-decision`、請求檔也讀得到。把這一項掛在事由上，那一種就漏掉了。判準是那一欄的語意，用是或否答得完：這一欄問的是不是一個已經執行完的動作要怎麼收？答是就要交代，不必再判斷那個動作有多重——本判準問的只有它做完了沒有，不是後果輕重。phase agent 那一側的契約要它把事後才醒悟的那一種寫成「這個已經做掉的動作要怎麼補救」，那是典型寫法而不是比對樣式：寫成「我已經把 X 推上去了，現在怎麼收」的，字面上不是那一句，語意上是同一件事，照樣要交代下面兩件事。拿字面去比對的後果就是這兩件事漏了交代，而編排端沒有第二個通道知道這是一件既成事實——它不讀請求檔，也不讀畫面。材料的分工與上面那組相同，只是位置不同：那一組的材料在畫面上，這一種在請求檔裡，而讀那份檔案的是本 subagent，編排端手上只有畫面上那一行狀態標記。

- **這次要定的是既成事實的補救，不是某件事該不該做**：一份「怎麼補救」的選項清單，形狀與一份「該不該做」的選項清單相同，上面那三項寫得再完整也分不出是哪一種。這個性質是編排端的分流條件——這一種它要即時報給使用者，另一種不必；不寫出來，一個已經發生的既成事實會被當成一次普通的定案靜靜送回去，兩邊都不會有任何錯誤訊息。「即時報」不等於「不升級」：編排端的升級規則不承接的只是那個已經做掉的動作本身，補救不是。補救要授權一個新動作時（典型是對共用分支 force push 改寫歷史），那個新動作在它那一端照一般判準重新判定；兩個補救方案之間是價值取捨時（典型是要不要接受另一個 phase 未推送的工作遺失），那本來就只有使用者拍板得了。所以每個補救選項的後果都要寫清楚它要動到什麼、會不會出這個 phase 的範圍、代價落在誰身上，不要壓縮成「照推薦做就好」——那一端就是靠這幾句判斷該自決還是停下問人
- **那個動作的落點，以及它已經波及到哪裡**：動到的是哪個檔案、跑掉的是哪一條指令、對哪個遠端或外部服務做了什麼，以及影響有沒有出這個 phase 的範圍；粒度與上面那顆「那個框要放行的具體動作落點」相同，指不出來時據實說指不出來，不用推測補齊。這一項與上一項同樣是自己重新組織過的敘述，不是把請求檔那五個欄位的原文帶回去
...
自我檢測（顆粒度）：把這一段決定本身以外的轉述整段刪掉，編排端還做得出同一個決定、走得到同一個分支、交得出它據以行動所需的內容嗎？三題都還是，這一段就超出了粒度，改成給證據位置。第三面是前兩面問不到的：既成事實的落點與波及範圍刪掉之後，分支與定案都不變，可是編排端要即時報給使用者的正是那些內容，刪了它只報得出「有一個既成事實」。受詞是上面那條判準原則，不是一份動作面清單；要一個錨點對齊直覺就用「同一顆鍵還按不按得下去」，但判準是那條原則本身。
