...
- 觸發本次調查的具體事由。所有事由都以 orchestrator 已經完成 A 與 B 分流、判定為 B 路徑為前提：事由說明的是這次從哪個入口轉進來，本身不是派工的充分條件。依入口分三種：
  - **決策請求**：提供 phase agent 送出的原文（問題、已經試過什麼、選項與後果、推薦、設計文件依據）。其中「設計文件依據」一欄，缺席、或內容是 phase agent 自陳「設計文件沒有交代」，兩種都合法，都不算短缺。這一欄的用途是給比對定一個起點：欄位指到設計文件某一段時，那一段就是本 subagent 拿調查所得去比對的起點；欄位缺席或自陳沒有交代時，這件事本身就是「設計文件在此有缺口」的訊號，值得寫進推薦理由
