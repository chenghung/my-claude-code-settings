...
事由是 `need-decision`、而本回合請求檔的「問題」欄位問的是一個已經執行完的動作要怎麼收時，推薦中另須以自己的話交代下面兩件事。判準是那一欄的語意，用是或否答得完：這一欄問的是不是一個已經執行完的動作要怎麼收？答是就要交代，不必再判斷那個動作有多重——本判準問的只有它做完了沒有，不是後果輕重。phase agent 那一側的契約要它把事後才醒悟的那一種寫成「這個已經做掉的動作要怎麼補救」，那是典型寫法而不是比對樣式：寫成「我已經把 X 推上去了，現在怎麼收」的，字面上不是那一句，語意上是同一件事，照樣要交代下面兩件事。拿字面去比對的後果就是這兩件事漏了交代，而編排端沒有第二個通道知道這是一件既成事實——它不讀請求檔，也不讀畫面。材料的分工與上面那組相同，只是位置不同：那一組的材料在畫面上，這一種在請求檔裡，而讀那份檔案的是本 subagent，編排端手上只有畫面上那一行狀態標記。
