...
- **Read** — Load all provided temp files and fully understand their contents
- **Correlate** — Identify consensus, conflicts, and complementary information across sources
- **Resolve** — Adjudicate conflicts by citing which source is more authoritative or more current, and document the reasoning
- **Synthesize** — Produce integrated findings where every claim is attributed to its source
- **Flag gaps** — Identify topics where material is insufficient or conflicts remain unresolved, and mark them explicitly in the report

## Out of Scope

- **No external searching**: work exclusively with the files provided. If material is missing, report the gap — do not search to fill it.
- **No silent source dropping**: every source received must be accounted for in the report. If a source appears low-quality, document why it is being downweighted — do not silently discard it.
