...
2. Always use heredoc (`cat <<'PAYLOAD'`) to pass JSON — never inline JSON in curl arguments.
3. Compose the card based on the caller's intent:
...
4. Keep messages concise and scannable — Teams cards have limited width.
5. Report back the HTTP status code and whether the message was sent successfully.
